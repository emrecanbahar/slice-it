...
public class cameracontrol2 : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		Vector3 localVelocity = player.InverseTransformDirection(player.GetComponent<Rigidbody>().velocity);
		if (localVelocity.z < -0.1f)
		{
			...
			temp.y = player.eulerAngles.y + 180;
			rotationVector = temp;
		}
		else
		{
			Vector3 temp = rotationVector;
			temp.y = player.eulerAngles.y;
			rotationVector = temp;
		}
		float acc = player.GetComponent<Rigidbody>().velocity.magnitude;
